...
class feat_extraction_layer(nn.Module):
    def __init__(self, use_normal = True):
        ...
        self.sa1 = PointNetSetAbstraction(npoint = 10000, radius = 0.1, nsample = 32, in_channel = in_channel, mlp = [16, 16, 32], group_all = False)
        self.sa2 = PointNetSetAbstraction(npoint=10000, radius=0.2, nsample=32, in_channel = 32, mlp=[32, 64],
                                          group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=10000, radius=0.4, nsample=32, in_channel=64, mlp=[64, 64],
                                          group_all=False)
